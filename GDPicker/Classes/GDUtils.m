...
//        NSLog(@"开始转化:%d", i);
        ...
        [[PHImageManager defaultManager] requestImageForAsset:each targetSize:PHImageManagerMaximumSize contentMode:PHImageContentModeDefault options:requestOptions resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
//            NSLog(@"得到结果:%d", i);
...
+ (void)burstimgsWithPhasset:(PHAsset *)phasset completion:(void (^)(NSMutableArray *images))completionBlock {
    ...
    for (int i = 0; i<burstCount; i++) {
        ...
        CGSize targetSize = PHImageManagerMaximumSize;          //原图
        targetSize =  [GDUtils sizeMaxWidth:300.0f withAsset:each];//尺寸大容易崩溃
